...
export default function WineWithDedication() {
  const { addToCart } = useCart();

  const { data: products = [], isLoading } = useQuery({
    queryKey: ["products", "יין עם תמונה והקדשה אישית"],
    queryFn: async () => {
      const allProducts = await base44.entities.Product.filter({ active: true });
      return allProducts.filter(p => {
        const categories = Array.isArray(p.category) ? p.category : [p.category];
        return categories.includes("יין עם תמונה והקדשה אישית");
      });
    }
  });
  ...
  return (
    <div className="min-h-screen bg-amber-50">
      {/* Header */}
      <section className="relative py-16 bg-gradient-to-br from-orange-400 to-orange-500">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <h1 className="text-4xl sm:text-5xl font-bold text-white mb-4">
              יין עם הקדשה אישית ותמונה
            </h1>
            <p className="text-xl text-white/90">
              מתנה מושלמת עם נגיעה אישית 🍷💝
            </p>
          </motion.div>
        </div>
      </section>
